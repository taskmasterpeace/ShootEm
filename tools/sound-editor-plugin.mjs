...
export function soundEditorPlugin() {
  return {
    ...
    async configureServer(server) {
      ...
      const { SOUND_SPECS } = await import(toolUrl('sound-specs.mjs'));
      const {
      ...
      server.middlewares.use('/api/sound', async (req, res, next) => {
        ...
        try {
          ...
          if (req.method === 'GET' && url === '/specs') {
            ...
            const specs = {};
            for (const [name, s] of Object.entries(SOUND_SPECS)) {
          ...
          if (req.method === 'POST' && url === '/gen') {
            ...
            if (!name || !SOUND_SPECS[name]) return send(res, 400, { error: `unknown sound '${name}'` });
            ...
            const key = loadKey();
            const cat = SOUND_SPECS[name].cat;
            const desc = (text || SOUND_SPECS[name].desc).trim();
            const duration = Number(dur) || SOUND_SPECS[name].dur || 1;
          ...
          if (req.method === 'POST' && url === '/approve') {
            ...
            copyFileSync(src, liveWav);
            try { wavToOgg(liveWav); } catch (e) { return send(res, 500, { error: `wav copied but ogg encode failed: ${e.message}` }); }
            if (text && text.trim()) {
              const overrides = readJson(PROMPT_OVERRIDES, {});
          ...
          if (req.method === 'POST' && url === '/save') {
            ...
            if (wavBuf.length < 44) return send(res, 400, { error: 'payload is not a WAV' });
            if (asVariant) {
              mkdirSync(VARIANTS_DIR, { recursive: true });
              ...
              try { wavToOgg(wav); } catch (e) { return send(res, 500, { error: `wav written but ogg failed: ${e.message}` }); }
              return send(res, 200, { ok: true, variant: i, url: `/audio/variants/${name}-${i}.wav?t=${Date.now()}` });
            }
            const liveWav = join(AUDIO_DIR, `${name}.wav`);
            writeFileSync(liveWav, wavBuf);
            try { wavToOgg(liveWav); } catch (e) { return send(res, 500, { error: `wav written but ogg failed: ${e.message}` }); }
            return send(res, 200, { ok: true, live: `/audio/${name}.ogg?t=${Date.now()}` });
          }
