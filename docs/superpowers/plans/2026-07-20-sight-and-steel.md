...
- [x] **Step 1 — the upstairs fishbowl [BUG, hours]. DONE 2026-07-21.** `world.ts:2902,3182` set `s.pos.y = 4` for `floor === 1`. `perception.ts:94` read `if (s.pos.y > 3 && eyes.some(within range)) return true` — **no cone, no LOS, no smoke** — so anyone on a second storey was visible to the entire enemy team out to 65 u through every wall and roof. **Fixed** by guarding on floor: `s.pos.y > 3 && (s.floor ?? 0) !== 1 && …` — a jet or jump-trooper (floor 0) still skylines; an upstairs body (floor 1) falls through to the normal cone+LOS path. `tests/fishbowl.test.ts` (3) proves both halves; the guarded trio (visibility/visionfade/culling) stays green unedited. Step 2 (upper-layer `grid2` LOS) remains for true upstairs-vs-upstairs sight.
- [x] **Step 2 — upstairs has no walls, to the eyes [BUG, hours]. DONE 2026-07-21.** `losClear(grid, a, b, y)` took **one layer** and never consulted `grid2`, so upstairs soldiers read the ground plan by accident. **Fixed:** added `losClearUpper` (`map.ts`, marches `grid2` via `blocksShotUpper` at the 5.4 nest band) and `perceivesNow` now rides it whenever both eye and target are `floor === 1` — an optional `grid2` param threaded from `updateLastSeen` and `cullSnapshotFor`. `tests/upperlos.test.ts` (3): upper wall hides, open floor / upper window sees, and the ground path stays untouched when `grid2` is absent.
