...
- [ ] **Step 1: Write failing catalog tests** proving five unique IDs, exact verb/site/class combinations, eight prints, determinism, and no `frontId`.
...
- [ ] **Step 2: Run `npx vitest run tests/science-presets.test.ts`; confirm RED** because the module does not exist.

- [ ] **Step 3: Implement the frozen catalog and helper** by calling `prepareScienceMission(preset.seed, null, 8, preset.options)`.

- [ ] **Step 4: Run the focused test and `npx tsc --noEmit`; confirm GREEN.**

- [ ] **Step 5: Commit** with `feat: define science quick deploy missions`.
...
- [ ] **Step 1: Add failing presentation assertions** that `index.html` contains the labelled quick-deploy region and `sciencePresetCardHTML` emits one semantic button with `data-science-preset`, title, description, class, and tags for every preset ID.

- [ ] **Step 2: Run the focused test; confirm RED** on the missing region/render helper.

- [ ] **Step 3: Add the HTML region** immediately below `#mode-select` with heading, explanatory copy, and `#science-preset-cards`.

- [ ] **Step 4: Render the five buttons in `main.ts`** by joining `sciencePresetCardHTML`. Attach listeners through each stable `data-science-preset` value. On click set Science mode, preset class/theme, issue weapons, eight prints, clear front state, assign `queuedScienceLaunch = prepareSciencePreset(preset)`, and call `startGame()`.

- [ ] **Step 5: Add production styles** for amber operation cards, tag pills, 10px corners, hover/focus/disabled states, >=44px targets, responsive wrapping, and reduced motion.

- [ ] **Step 6: Run focused tests, TypeScript, and ESLint; confirm GREEN.**

- [ ] **Step 7: Commit** with `feat: add science quick deploy menu`.
...
- [ ] **Step 1: Start Vite and inspect the shelf** at desktop and narrow widths. Confirm all five cards are readable, clickable, and do not cover existing menu controls.

- [ ] **Step 2: Launch K9 House Clear** and confirm Hunt + officer villa + eight prints + local dog/K9 controls.

- [ ] **Step 3: Return and launch a non-K9 preset**; confirm its expected verb/site/class and no campaign mutation. Check console errors.

- [ ] **Step 4: Document exact cards and verified behavior.**
