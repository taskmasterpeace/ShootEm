...
| 4 | **Shadowstep** | assassin · blink | Blink behind, stab | Departure point holds an explosive decoy — **blows on touch** | Chasing him IS the trap | Shoot the decoy at range | T1 | ✅ (blink-stab + the LIVE MINE shipped in the fifth wave-2 batch: he blinks behind the aimed mark and stabs, and the DEPARTURE POINT holds a live mine — chasing him IS the trap, and shooting it at range is the answer (law-tested). Threat MEASURED 14.3s vs 4) 🎮 pilot: Q = BLINK-STAB |
...
| 6 | **Leviathan** | boss · massive | Terrain-flattening sweeps | **Belly flop**: map-scale leap, shadow-telegraphed landing, rim shockwave | Scatter from the shadow — and he's soft mid-air | Hit him in the air (the AA window) | **T3** | ✅ (sweeps + the BELLY FLOP SHIPPED: the forward arc flattens soldiers AND their cover (DESTRUCTION does the paperwork, law-tested); the flop is a map-scale leap whose SHADOW marks the landing for 1.5s — zero damage until the promise lands ON the mark, the RIM eats the shockwave, and the scatter law is real: the stander eats it, the reader walks (law-tested). SOFT MID-AIR: every round that finds him between leap and landing bites 1.6x deeper — the AA window in code. Threat MEASURED 26.3s vs 12) 🎮 pilot: Q = THE FLOP |
...
| 19 | **Cataclysm** | siege boss · massive | Huge, slow area slams | While he lives, seismic eruptions fire map-wide, **worsening the longer he's up** | A DPS check that punishes stalling — the announcer counts the quakes you survived | All-in focus | T2 | ✅ (the slam + THE WORSENING SHIPPED: huge slow area slams up close — and while he LIVES the map quakes, eruptions finding the living ANYWHERE (distance is not safety, law-tested), on an interval that shrinks from 6s at birth to a 2s floor at 90s up. The DPS check is arithmetic: a 20s kill endures ~4 quakes, a 90s stall endures ~19 (law-tested) — and the announcer counts every fifth one out loud. Threat MEASURED 12.3s vs 8) 🎮 pilot: Q = THE SLAM |
