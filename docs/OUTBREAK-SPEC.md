...
> **BUILD STATUS (2026-07-20):** the launch trio is LIVE. **B** cycles the loaded round Standard Ball → AP → Incendiary (`Soldier.ammoType`; deterministic via `PlayerCmd.cycleAmmo`, so replays reproduce it). AP threads issued plate for −25% soft damage on ballistic weapons (bullet/shell — energy weapons unaffected; iron molt & LSW identity armor stay exempt). Incendiary burns the body it lands on — denying reanimation like a blast — and hits any ZedKind for ×1.6 at a −15% cost against the living. Gods skip the cycle (a firebrand doesn't fumble ammo boxes). The magazine counter reads `30 / 90 · AP` / `· INC`. `tests/ammo.test.ts` (5). **FULL §11.1 ROSTER SHIPPED (2026-07-20):** the cycle now runs ball → AP → INC → **TRC** (marks the struck target, loud) → **SUB** (−20% dmg / −25% range, quiet) → **EXP** (×1.5 vs bare flesh, ×0.65 vs armor·undead) → **BNR** (chemical corpse denial without fire, −40% dmg). `tests/ammo.test.ts` (8). **§11.2 HUD RATINGS SHIPPED (2026-07-21):** under the mag counter the loaded round now reads its **role + PEN / NOISE / FIRE / CORPSE-denial** as 3-notch mono bars (`AMMO_INFO` in `data.ts` is the single source, shared with the B-cycle toast; `hud.ts` `#ammo-info`; `tests/ammo-info.test.ts`). Live-verified: ball `GENERAL PEN▮▯▯`, AP `ARMOR PEN▮▮▮`, INC `BURN FIR▮▮▮ COR▮▮▮`. **§11.3 SEPARATE POOLS SHIPPED (2026-07-21):** special rounds reload from their OWN per-type pools (`AMMO_INFO.pool`: AP/TRC/SUB 60 · INC/EXP 40 · BNR 20 — "expensive, limited"); ball rides the classic reserve; an empty pool falls the selector back to ball with a `DRY — BALL` callout; ammo crates refill the pools; the mag counter's reserve number is what the NEXT reload draws (`30 / 35 · AP`). Bots never cycle → classic path untouched. `tests/ammo-pools.test.ts` (5). REMAINING: mixed magazines + the "next round" cue (§11.3 advanced, explicitly a later feature), and the full §11.4 material-fire interaction (waits on W7.3).
...
> **BUILD STATUS (2026-07-20):** the triangle CLOSES — all three vertices are LIVE and counter each other. **STRIKE** — every soldier without a returning axe carries a universal Combat Knife on **F** (34 dmg, 2.2u reach), driving the existing windup→90°-arc→stagger swing and sharing the fire clock. **GUARD** — held **V** raises a brace over a 150° frontal cone: a facing STRIKE lands only 12% and PARRIES (staggers + shoves the attacker — *Guard beats Strike*), while a flank/rear blow slips past. Guard costs ~10 stamina/s, pauses regen, slows to 0.45×, and lowers your own weapons. **GRAPPLE** — **Z** is a close grab that BYPASSES and drops a raised guard (*Grapple beats Guard*) and pins the target in a rooted hold; a target already mid-STRIKE stuffs the grab (*Strike beats Grapple*); the pinned body mashes MOVE to break early and rebounds on the grabber. Events: `melee_block`, `grabbed`, `grab_break`. `tests/melee.test.ts` (15 new). **IMPACT CHARGE** (§13) is also LIVE: **hold F** to wind up the knife STRIKE (bands 0-30% quick ×1 / 31-70% heavy ×1.6 / 71-100% maximum ×2.4 / overhold = fumble ×1.2 + stamina bleed), release to commit; a tap stays an instant quick strike; the meter reads on the action line. **BITE STRUGGLE** (§15.5) is LIVE too: ~1 in 4 shamblers latch on (`beginBiteStruggle`) — the grip gnaws Viral Load, failing to break in time is a full bite (damage + infection injection), mashing MOVE escapes, a broken clinch grants ~1s re-grab immunity, sprinters clamp briefly / brutes longer, gods & ascendants are immune; HUD banner switches to **BITE STRUGGLE**. REMAINING: the PvP **Control Struggle** best-of-three needle-vs-zone minigame + rear-control outcome menu (§14.2/§15) — the zombie survival variant (Bite Struggle) is the shipped half of §15. Simultaneous-action priority (§12.1) resolves today by swing timing; explicit startup/active-frame arbitration is a later pass.
