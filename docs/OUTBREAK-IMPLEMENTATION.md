...
| 16 | Combat UI & terminology | 🔨 | approved labels live (STRIKE/GUARD/GRAPPLE, IMPACT CHARGE, BITE STRUGGLE, CLEAN→EXPOSED→INFECTED→CRITICAL, GRABBED); near-the-action feedback (charge meter, break meter, biohazard chip). **§16.3 icon language SHIPPED** — `src/client/icons.ts`: all 11 spec concepts as one inline-SVG vocabulary (stroke=currentColor, no emoji, `tests/icons.test.ts`); every live surface wears it (guard/impact/rear on the ability-hint, escape on the struggle banner, infection on the viral chip + biohazard obj-chips, AP/INC on the ammo readout). corpse/rising now MOUNTED on the §6 corpse-proximity chip (nearest incubating body ≤12u: `BODY 5u · 14s` behind the body-with-timer; the final 2s flips it red to `NAME IS RISING` behind the rising silhouette); strike/grapple defined-and-waiting for the §16.2 stance line. Remaining §16: audio stingers priority table (§16.4 partial) |
