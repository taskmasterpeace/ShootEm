...
| 16 | Combat UI & terminology | 🔨 | approved labels live (STRIKE/GUARD/GRAPPLE, IMPACT CHARGE, BITE STRUGGLE, CLEAN→EXPOSED→INFECTED→CRITICAL, GRABBED); near-the-action feedback (charge meter, break meter, biohazard chip). **§16.3 icon language SHIPPED** — `src/client/icons.ts`: all 11 spec concepts as one inline-SVG vocabulary (stroke=currentColor, no emoji, `tests/icons.test.ts`); every live surface wears it (guard/impact/rear on the ability-hint, escape on the struggle banner, infection on the viral chip + biohazard obj-chips, AP/INC on the ammo readout). strike/grapple/corpse/rising defined-and-waiting for their surfaces (§16.2 stance line, a §6 HUD proximity chip). Remaining §16: audio stingers priority table (§16.4 partial) |
