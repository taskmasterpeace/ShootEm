...
| 14 | Rear grabs & control states | 🔨 | the GRAPPLE pins, and a **REAR TAKEDOWN** now finishes it — a second Z executes the pinned body (heavy AP overkill → a clean kill, not a crawl; gods exempt). `tests/takedown.test.ts`. The rest of the outcome menu (choke/disarm/throw/human-shield, §14.2) is design |
