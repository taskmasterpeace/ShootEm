...
| Fire / burnable wood | 🔨 | **FIELD FIRE Phase 1 + 2a SHIPPED 2026-07-22.** `flammable` is CONSUMED: an incendiary round on flammable cover, or an incendiary grenade's splash, lights the ground (`world.fires`, tile-based). **Phase 2a — THE FLAMETHROWER LAYS FIRE:** a splashless flame STREAM (the F-3 Flamer, the AR-606 under-barrel burp — NOT the fireball-splash gods) now washes the flammable GROUND it sweeps low over, lighting a whole swept swath, not just incendiary rounds on cover (the projectile-step hook, `def.tracer === 'flame' && !def.splash && y < 2`). A burning tile scorches who stands in it (`FIRE_DPS`), SPREADS to its flammable orthogonal neighbours once (grass carries, metal/masonry don't), burns down over ~5.5s, and the renderer licks flame + smoke off each tile. Deterministic — no RNG, empty-early-out keeps fire-free replays byte-identical (the FULL 2243-test suite + a determinism trace green; no seed-pinned balance arena moved). `world.igniteTile/igniteAt/stepFires`; `tests/fire.test.ts` (8, incl. "the stream lit grass it never stood on" + a bare-ground no-op); live-verified on the savanna — a swept fire fan, proof `docs/reference/field-fire-flamer.png`. REMAINING: house/wood-frame **structural** burn-through (Phase 2b). BACKLOG W7.3 |
...
| Corpse burning / neutralization meter | 🔨 | **TWO paths live now.** Direct: an incendiary/BNR round near a corpse fills its `burn` meter → neutralized (§6.1, shipped 2026-07-20). FIELD: a corpse lying in **field fire** (W7.3, 2026-07-22) burns down to neutralized on its own — the incendiary→ground→fire→corpse fold Robert wanted. `tests/fire.test.ts` pins the field path. REMAINING: a visible per-corpse burn meter on the HUD |
