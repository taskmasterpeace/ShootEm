...
| Fire / burnable wood | 🔨 | **FIELD FIRE Phase 1 SHIPPED 2026-07-22.** `flammable` is CONSUMED now: an incendiary round on flammable cover, or an incendiary grenade's splash, lights the ground (`world.fires`, tile-based); a burning tile scorches who stands in it (`FIRE_DPS`), SPREADS to its flammable orthogonal neighbours once (grass fields carry, metal/masonry don't), burns down over ~5.5s, and the renderer licks flame + smoke off each tile. Deterministic — no RNG, empty-early-out keeps fire-free replays byte-identical. `world.igniteTile/igniteAt/stepFires`, `tests/fire.test.ts` (6, incl. a determinism trace); live-verified on the savanna (25-tile grass fire, player HP scorched 100→51, 0 errors). REMAINING: house/wood-frame structural burn-through + the flamer's direct stream igniting ground (Phase 2). BACKLOG W7.3 |
