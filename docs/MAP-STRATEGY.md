...
| **E · Vehicle theaters** | 600–900u per axis | jet corridors, armor maneuver, rotor insertion, littoral and submarine war | deterministic biome generators with map-owned geometry |
...
- **Persistence truth:** Operation metadata survives Map Maker
  serialization. Carriers remain sites/strategic effects; the Barracuda is a
  real playable submarine with deep-water staging, sonar, and torpedoes.

## 2.10 Vehicle-scale theaters — SHIPPED

Military Operations can now select six independent deterministic battlefields
whose dimensions belong to the map instead of the old global 300u assumption:

| Theater | Size | Operational read |
|---|---:|---|
| City | 600×600u | block grid, broad avenues, enterable urban cover |
| Desert | 900×900u | long armor sightlines, dunes and isolated compounds |
| Countryside | 900×900u | fields, hedgerows, villages and jet approach lanes |
| Mountain | 600×900u | a long pass, switchbacks and constrained high ground |
| Coastal | 900×600u | land maneuver beside connected shallow/deep water |
| Ocean | 900×900u | surface lanes, deep patrol routes and island objectives |

Every theater owns legal spawn pads and deterministic ground, surface, deep,
rotor-support, rotor-insertion and air routes as its domains require. Operations
select the theater from the authored site and manifest; Map Maker imports,
edits and exports the same rectangular geometry.

The shared elevation law is now exactly four semantic levels: **Ground,
Building, Sky, Clouds**. Submarine depth is deliberately separate from air
elevation. The Shrike attack helicopter and Condor transport helicopter use
the lower three air levels; fixed-wing aircraft use all four. Coastal and
Ocean field the Pike gunboat and Barracuda attack submarine, including
deep-only diving, team sonar and torpedo-only submerged damage.

Production evidence is generated by `npm run test:vehicle-scenarios`:
210 foundation route/duel/combined-arms probes, 100 rotorcraft support and
insertion probes, and 20 submerged submarine duels. Reports live under
`docs/reference/vehicle-theaters/` and include first-contact timing, route
completion, shots, hits, losses, depth transitions, stuck incidents and
side-rate balance.
