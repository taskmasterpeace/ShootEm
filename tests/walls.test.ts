...
import { GRID, T_COVER, T_DOOR, T_DOOR_OPEN, T_LADDER, T_METAL, T_OPEN, T_SLIT, T_WALL, T_WATER, TILE, WORLD, generateMap } from '../src/sim/map';
...
describe('invisible walls: the render-coverage invariant', () => {
  ...
  it('the grid speaks only known tile types — new types must enroll here AND in the renderer', () => {
    const known = new Set([T_OPEN, T_WALL, T_COVER, T_WATER, T_SLIT, T_DOOR, T_DOOR_OPEN, T_METAL, T_LADDER]); // slits: stacked boxes · doors: live slabs · metal: steel instancing · ladders: rail+rung mesh (walkable ground)
    for (const m of allMaps()) {
