...
const VEHICLE_PARTS: Record<VehicleKind, string[]> = {
  ...
  // the new air program — each airframe now has its OWN silhouette (buildVehicle)
  gunship: ['turret', 'gunRecoil', 'thrustL', 'thrustR', 'podL', 'podR'], // Warhawk — straight-wing gun jet
  ...
  golfcart: [], bicycle: [], train: [], subway: [], musclecar: [], roadster: [], rallytruck: [], racetruck: [], hotrod: [],
...
describe('visual: props', () => {
  it.each(['rock', 'tree', 'crate', 'bunker', 'clone_bay', 'silo', 'flare_stack', 'crane', 'wreck', 'memorial'])('%s prop builds visibly', (type) => {
...
describe('visual: no purple, ever', () => {
  ...
  it('no gadget, pickup, prop, or structure material is purple', () => {
    ...
    for (const p of ['rock', 'tree', 'crate', 'bunker', 'clone_bay', 'silo', 'flare_stack', 'crane', 'wreck', 'memorial']) assertNoPurple(buildProp(p, 1.2), p);
