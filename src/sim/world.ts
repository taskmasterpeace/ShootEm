...
import { isTrainable, practise, skillEdge, skillForWeapon, skillLevel, practiceOf } from './skills';
import {
...
import {
  ...
  type Vehicle, type VehicleDef, type VehicleKind, type VehicleSystems, type WeaponId, type ZedKind, isIron, isBoard, type IronKind, type SkillId, type SoldierStats } from './types';
...
export interface WorldOptions {
  ...
  /** THE 8, from the career ledger. Humans were hardcoded to a flat 5 here,
   *  which made every statMul/statQuick hook return EXACTLY 1.000 for the
   *  person playing — eight named stats that multiplied by one. They arrive as
   *  an OPTION, like `papers` and `rank`, so stats can only ever move BETWEEN
   *  matches and the tick loop stays a pure function of its inputs. */
  startingStats?: Partial<SoldierStats>;
  matchMinutes?: number;
...
export class World {
  ...
  /**
   * PRACTISE, AND SAY SO WHEN IT MATTERS.
   *
   * Every trade in the game was levelling in total silence: `practise()` is
   * pure and emits nothing, and the SimEvent union had no member for it, so the
   * client was structurally deaf to the one thing that was actually improving.
   *
   * A crossing is knowable for free — `practise` already returns the new total,
   * so it is one comparison. It is announced ONLY on a crossing, which there
   * are five of per trade in a whole career, so the moment can afford to be
   * ceremonial instead of becoming a stream of numbers nobody reads.
   *
   * The emit lives HERE and not in skills.ts: that file's purity (no rng, no
   * clock, no DOM, no events) is the reason it is testable.
   */
  practiseAt(s: Soldier, id: SkillId, amount: number): void {
    if (!isTrainable(s)) return;
    const before = skillLevel(practiceOf(s, id));
    const after = skillLevel(practise(s, id, amount));
    if (after > before) {
      this.emit({ type: 'skill_band', soldierId: s.id, text: id, amount: after, pos: { ...s.pos } });
    }
  }
  ...
  practiseSeat(driver: Soldier, def: VehicleDef, dt: number): void {
    ...
    if (def.flies || def.hover) this.practiseAt(driver, def.flies && !def.hover ? 'jet' : 'helicopter', 1);
    else if (def.boat) this.practiseAt(driver, 'boat', 1);
    else this.practiseAt(driver, 'tank_driver', 1);
  }
  ...
  constructor(public opts: WorldOptions) {
    ...
    this.map = scienceLayout?.map ?? opts.map ?? ((opts.operation && opts.operationManifest && opts.operationInventory
  ...
  addSoldier(name: string, classId: ClassId, team: Team, kind: SoldierKind, loadout?: Loadout): Soldier {
    ...
    if (kind === 'human') {
      ...
      // the career's eight, over the neutral 5s
      if (this.opts.startingStats && s.stats) s.stats = { ...s.stats, ...this.opts.startingStats };
    }
    ...
    for (const s of this.soldiers.values()) {
      ...
      if (led > 0) {
        ...
        if (ledBy) {
          ...
          if (ledBy.cmdPractice >= 5) { ledBy.cmdPractice = 0; this.practiseAt(ledBy, 'commander', 1); }
        }
      }
      ...
      for (const s of this.soldiers.values()) {
        ...
        if (perceivesNow(this.map.grid, eyes, this.pinged, s, this.scoutRange(eyes, range), this.smokeBlobs,
          ...
          if (fresh) {
            ...
            if (best) this.practiseAt(best, 'scout', 1);
          }
        }
      }
    }
      ...
      switch (g.type) {
        ...
        case 'drone': {
          ...
          if (g.piloted) {
            ...
            if (owner.dronePractice >= 2) { owner.dronePractice = 0; this.practiseAt(owner, 'drone_pilot', 1); }
  ...
  private buildingAt(x: number, z: number): boolean {
    ...
    return t === T_WALL || t === T_SLIT || t === T_DOOR || t === T_METAL || t === T_METAL_DOOR
      || (t >= T_THIN_WALL_H && t <= T_THIN_WALL_HV
        && blocksShot(this.map.grid, x, z, 2, this.map.geometry));
  }
  ...
  applyCmd(s: Soldier, cmd: PlayerCmd, dt: number) {
    ...
    if (s.sprinting) {
      ...
      if (s.navPractice >= 4) { s.navPractice = 0; this.practiseAt(s, 'navigator', 1); }
    }
    ...
    for (const { victim, d } of caught.slice(0, MELEE_MAX_TARGETS)) {
      ...
      if (s.kind === 'dog') {
        ...
        if (handler) {
          ...
          if (handler.dogPractice >= 3) { handler.dogPractice = 0; this.practiseAt(handler, 'dog_handler', 1); }
        }
      }
  ...
  private tryStairs(s: Soldier): boolean {
    ...
    const [tx, tz] = worldToTile(this.map.geometry, s.pos.x, s.pos.z);
    s.floor = target;
    ...
    s.pos.x = center.x + stair.x * side * this.map.geometry.tile * 0.4;
    s.pos.z = center.z + stair.z * side * this.map.geometry.tile * 0.4;
    ...
    if (this.hasEquip(s, 'fieldRepair')) {
      for (const v of this.vehicles.values()) {
        ...
        if (Math.hypot(v.pos.x - s.pos.x, v.pos.z - s.pos.z) < VEHICLES[v.kind].radius + 2.5) {
          ...
          this.practiseAt(s, 'mechanic', 4); // the trade is earned on the hulls you fix
  ...
  private tryDoor(s: Soldier): boolean {
    ...
    for (const reach of [tile * 0.6, tile * 1.3]) {
      ...
      this.toggleDoorTile(floor * area + idx, s.id);
      return true;
    }
    return false;
  }
  ...
  private toggleDoorTile(packed: number, soldierId: number) {
    const area = this.map.geometry.cols * this.map.geometry.rows;
    ...
    const pos = tileToWorld(this.map.geometry, idx % this.map.geometry.cols, Math.floor(idx / this.map.geometry.cols));
    this.emit({
      type: 'door', tile: packed, soldierId,
      pos: { ...pos, y: floorHeight(floor) },
    });
  }
  ...
  /** Broken panes packed as floor*map-area+idx. Cumulative and idempotent like
  ...
  shatterWindowAt(x: number, z: number, floor = 0): boolean {
    ...
    const packed = floor * cols * rows + idx;
    if (!this.glassChanges.includes(packed)) this.glassChanges.push(packed);
    this.emit({
      type: 'glass',
      pos: { ...tileToWorld(this.map.geometry, tx, tz), y: floorHeight(floor) + 1.5 },
      tile: packed,
    });
    return true;
  }
    ...
    if (!parked && !def.immobile && !spooling) {
      ...
      if ((def.flies && !stormGrounded) || (def.digs && v.burrowed)) {
        ...
        if (def.digs && Math.abs(throttle) > 0.1 && this.time >= v.nextDigAt) {
          const aheadX = v.pos.x + Math.cos(v.yaw) * (r + this.map.geometry.tile * 0.6) * Math.sign(throttle);
          const aheadZ = v.pos.z + Math.sin(v.yaw) * (r + this.map.geometry.tile * 0.6) * Math.sign(throttle);
  ...
  private stepSticky(id: number, p: Projectile, px: number, py: number, pz: number): boolean {
    ...
    if (p.pos.y < 4.05 && isBlocked(this.map.grid, p.pos.x, p.pos.z, false, this.map.geometry)) {
  ...
  stepProjectiles(dt: number) {
    ...
    for (const [id, p] of this.projectiles) {
      ...
      if (!dead && def.damage > 0 && !def.training) {
        ...
        try {
          const layer = floorLayer(this.map, floor);
          const pane = tileAt(layer, p.pos.x, p.pos.z, this.map.geometry);
          if (isWindowTile(pane, floor > 0) && !windowIsBroken(pane, floor > 0) && thinTileBlocks(pane, p.pos.x, p.pos.z, floor > 0, this.map.geometry)
  ...
  damageSoldier(victim: Soldier, dmg: number, attackerId: number, weapon: WeaponId, viaLink = false, pierceArmor = false) {
    ...
    if (dmg > 0 && attackerId >= 0 && attackerId !== victim.id) {
      ...
      //
      // The rule now lives in ONE place — `isTrainable` in skills.ts — because
      // practice and RUST both have to obey it, and two copies of an exemption
      // is how the threat table breaks quietly six months from now.
      if (shooter && trains && shooter.team !== victim.team) this.practiseAt(shooter, trains, 0.5);
    }
    victim.hp -= dmg;
  ...
  private stepSpiderMine(id: number, m: Mine, dt: number) {
    ...
    if (!isBlocked(this.map.grid, nx, m.pos.z, false, this.map.geometry)) m.pos.x = nx; // slide along walls
    if (!isBlocked(this.map.grid, m.pos.x, nz, false, this.map.geometry)) m.pos.z = nz;
    if (tgt && td < 1.5) {
