 ...
 *  breaks — the "he just ducked behind the wall" trail. Felt range is 1–3s;
 *  2 keeps a corner-camper honest without turning walls into glass. */
export const SEEN_LINGER = 2;

/** Can this set of friendly eyes perceive enemy soldier `s` RIGHT NOW? */
export function perceivesNow(grid: Uint8Array, eyes: Soldier[], pinged: Set<number>, s: Soldier): boolean {
  ...
  return eyes.some((e) =>
    Math.hypot(s.pos.x - e.pos.x, s.pos.z - e.pos.z) < PERCEIVE_RANGE &&
