...
import { geometryLength, halfDepth, halfWidth, inBounds, tileIndex, worldToTile } from './map-geometry';
...
function distanceToHouse(map: GameMap, index: number, point: Vec3): number {
  ...
  const minX = house.tx * map.geometry.tile - halfWidth(map.geometry);
  const maxX = (house.tx + house.tw) * map.geometry.tile - halfWidth(map.geometry);
  const minZ = house.tz * map.geometry.tile - halfDepth(map.geometry);
  const maxZ = (house.tz + house.th) * map.geometry.tile - halfDepth(map.geometry);
...
export function buildingAtOrderPoint(map: GameMap, point: Vec3): number {
  const direct = houseAt(map.houses, point.x, point.z, map.geometry);
...
export function hostilesInK9Building(
  ...
  for (const soldier of soldiers) {
    ...
    if (houseAt(map.houses, soldier.pos.x, soldier.pos.z, map.geometry) === dog.k9BuildingId) result.push(soldier);
  }
  return result;
}
...
export function k9SearchWaypoints(world: World, dog: Soldier): readonly Vec3[] {
  ...
  const rooms = world.indoorTactics?.roomCenters.filter((center) =>
    houseAt(world.map.houses, center.x, center.z, world.map.geometry) === dog.k9BuildingId);
...
export function dogInsideAssignedBuilding(map: GameMap, dog: Soldier): boolean {
  return dog.k9BuildingId !== undefined
    && houseAt(map.houses, dog.pos.x, dog.pos.z, map.geometry) === dog.k9BuildingId;
}
...
export function closedDoorAhead(map: GameMap, dog: Soldier): number {
  ...
  for (const reach of [map.geometry.tile * 0.6, map.geometry.tile * 1.3]) {
    const [tx, tz] = worldToTile(
      map.geometry,
      dog.pos.x + Math.cos(dog.yaw) * reach,
      dog.pos.z + Math.sin(dog.yaw) * reach,
    );
    if (!inBounds(map.geometry, tx, tz) || tx < 1 || tz < 1 || tx >= map.geometry.cols - 1 || tz >= map.geometry.rows - 1) continue;
    ...
    if (isDoorTile(tile, upper) && !doorIsOpen(tile, upper) && (upper || tile !== T_METAL_DOOR)) {
      return dog.floor * geometryLength(map.geometry) + index;
    }
  }
  return -1;
}
